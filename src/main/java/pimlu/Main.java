...
public class Main {
    static final String usage = "usage:\n"
                    ...
                    + "  srt2vocab --read deck.tsv file1.txt f2.txt etc.txt"
    ...
    public static void main(String[] args) throws Exception {
        if(args.length == 0) {
            ...
            if(args.length < 3)
                die("need at least 3 arguments for --read, got "+args.length);
            init();
            DefCounter dc = new DefCounter();
            for(int i=2; i<args.length; i++) {
                System.out.printf("processing %s...\n", args[i]);
                Iterable<String> lines = SrtReader.readStream(
                        new FileInputStream(args[i])).lines()::iterator;
                for(String s : lines) {
